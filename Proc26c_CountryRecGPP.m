...
Path_Change    = '../output/25b_MMEDeffRec/';   % pixel-scale MME ΔRec
Path_Out       = '../output/26_CountryRecGPP/';
...
for s = 1:numel(SSPList)
    ...
    SSPName = SSPList{s};
    disp(['Processing country ΔRec (clean): ', SSPName]);

    % ---- 读取像元尺度 MME ΔRec ----
    Data = load(fullfile(Path_Change, SSPName, 'DeltaRec_MME.mat'));
    DeltaMap = Data.MMERec;
    ...
    % 回填国家尺度栅格（一个国家一个值）
    % ===============================
    CountryRec = nan(size(DeltaMap));
    for c = 1:numel(CountryList)
        CountryRec(CountryID == CountryList(c)) = CountryStat(c);
    end
    ...
    CountryTable = table(CountryList(:), CountryStat(:), ...
        'VariableNames', {'CountryID','DeltaRec'});

    save(fullfile(OutPath,'CountryDeltaRec.mat'), ...
        'CountryRec','CountryTable','-v7.3');
end

disp('Finished: Country-scale ΔRecovery (physically consistent).');
