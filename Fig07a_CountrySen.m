...
Path_Country   = '../output/26_CountrySenGPP/';
Path_Figure    = '../figure/';
...
for s0 = 1:numel(SSPList)
    D0 = load(fullfile(Path_Country,SSPList{s0},'CountryDeltaSen.mat'));
    v  = D0.CountrySen;
    v  = v(isfinite(v));
    AllVals = [AllVals; v(:)];
end
...
% ======================================================
ColorMap = [
     96  28   2;    % extreme decrease  (very dark brown)
    166  54   3;    % large decrease    (brown-orange)
    223 122  41;    % moderate decrease
    224 226 217;    % no change
    140 190 200;    % moderate increase (light blue-teal)
     70 140 180;    % large increase    (blue)
     30  90 150     % extreme increase  (deep blue)
] / 255;
...
for s = 1:numel(SSPList)
    ...
    % 读取国家尺度 Δlog(Sen)
    % ===============================
    Data  = load(fullfile(Path_Country,SSPName,'CountryDeltaSen.mat'));
    Class = Data.CountrySen;
...
% 合并 4 个面板
% ======================================================
Fig1 = imread([Path_Figure,'GeoCountrySen_ssp126.tif']);
Fig2 = imread([Path_Figure,'GeoCountrySen_ssp245.tif']);
Fig3 = imread([Path_Figure,'GeoCountrySen_ssp370.tif']);
Fig4 = imread([Path_Figure,'GeoCountrySen_ssp585.tif']);
...
imwrite(FigAll,[Path_Figure,'Fig07CountrySenChange.tif'], ...
    'Compression','LZW','Resolution',300);

delete([Path_Figure,'GeoCountrySen_*.tif']);
