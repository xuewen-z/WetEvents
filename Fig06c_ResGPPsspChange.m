...
% Path_RiskAnom = '../output/25_DeffResCMIP6GPP/';
...
for ss = 1:numel(SSPList)
    ...
    for m = 1:numel(ModeList)
        ...
        File = [Path_RiskAnom Mode '/' SSPName '/OmegaRes.mat'];
...
% 标题
text('String',[CapList{ss},IndName{ss},' ensemble mean resistance change'],...
...
print(Fig, '-dtiff', '-r300', [Path_Figure, 'ResGPPchange',SSPName,'.tif']); close(Fig);
...
% 合并面板
% ===============================
Fig1 = imread([Path_Figure,'ResGPPchangessp126.tif']);
Fig2 = imread([Path_Figure,'ResGPPchangessp245.tif']);
Fig3 = imread([Path_Figure,'ResGPPchangessp370.tif']);
Fig4 = imread([Path_Figure,'ResGPPchangessp585.tif']);
...
imwrite(Fig,[Path_Figure,'Fig08ResChange.tif'], ...
        'Compression','LZW','Resolution',300);

delete([Path_Figure,'ResGPP*.tif']);
