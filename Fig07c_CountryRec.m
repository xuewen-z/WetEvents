...
Path_Country   = '../output/26_CountryRecGPP/';   % ★ Rec
Path_Figure    = '../figure/';
...
for s0 = 1:numel(SSPList)
    D0 = load(fullfile(Path_Country,SSPList{s0},'CountryDeltaRec.mat'));
    v  = D0.CountryRec;
    v  = v(isfinite(v));
    AllVals = [AllVals; v(:)];
end
...
Qneg = prctile(NegVals,[33 66]);
Qpos = prctile(PosVals,[33 66]);

epsilon = 0.01;   % no-change 带宽（Δlog）
...
ColorMap = [
    ...
    210 180 210;    % moderate increase
    190 120 190;    % large increase
     80  20  80     % extreme increase
] / 255;
...
for s = 1:numel(SSPList)
    ...
    % 读取国家尺度 Δlog(Rec)
    % ===============================
    Data  = load(fullfile(Path_Country,SSPName,'CountryDeltaRec.mat'));
    Class = Data.CountryRec;
    ...
    ClassID(Class <= Qneg(1))                            = 1; % large dec
    ClassID(Class > Qneg(1) & Class <= Qneg(2))          = 2; % mod dec
    ClassID(Class > Qneg(2) & Class <  -epsilon)         = 3; % slight dec
    ...
    % 标题
    text('String',[CapList{s},IndName{s},' ensemble mean resilience change'],...
...
% 合并 4 个面板
% ======================================================
Fig1 = imread([Path_Figure,'GeoCountryRec_ssp126.tif']);
Fig2 = imread([Path_Figure,'GeoCountryRec_ssp245.tif']);
Fig3 = imread([Path_Figure,'GeoCountryRec_ssp370.tif']);
Fig4 = imread([Path_Figure,'GeoCountryRec_ssp585.tif']);
...
imwrite(FigAll,[Path_Figure,'Fig07CountryRecChange.tif'], ...
    'Compression','LZW','Resolution',300);

delete([Path_Figure,'GeoCountryRec_*.tif']);
